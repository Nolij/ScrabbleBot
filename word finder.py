...
def FindWords(Expression, Exclude = "^$"):
	...
	FilteredWords = [ Word for Word in Pool.map(Thread, WordList) if (Word != None) ]
	Pool.close()
...
if (__name__ == "__main__"):
	while (True):
		...
		#					Include = ("(?=(^([^%c]*%c){0,%d}[^%c]*$))" % (Letter, Letter, n, Letter)) + Include
		#	else:
			def GenerateLookAheads(BlankTiles = []):
				global Include
				for Tile in LetterMap.keys():
					...
					Include = '(' + Include
					if (len(BlankTiles) < Blanks): GenerateLookAheads(BlankTiles)
					del BlankTiles[-1]
